...
// .wrangler/tmp/bundle-uzOIZH/checked-fetch.js
...
// src/routes/auth.worker-1.ts
var MAX_ATTEMPTS = 5;
var RESEND_COOLDOWN_MS = 60 * 1e3;
var CODE_EXPIRY_MS = 10 * 60 * 1e3;
...
function isValidUTEmail(email) {
  return email.toLowerCase().endsWith("@utexas.edu");
}
__name(isValidUTEmail, "isValidUTEmail");
authRoutes.post("/send-code", async (c) => {
  ...
  ).bind(normalizedEmail, codeHash, Date.now() + CODE_EXPIRY_MS, Date.now()).run();
...
authRoutes.post("/verify-code", async (c) => {
  ...
  if (Date.now() > record.expires_at) {
    await c.env.DB.prepare("DELETE FROM verification_codes WHERE email = ?").bind(normalizedEmail).run();
  ...
  ).bind(Date.now(), normalizedEmail).run();
...
// src/routes/users.worker-1.ts
var userRoutes = new Hono2();
async function getAuthUser(authHeader, secret) {
...
userRoutes.post("/me/agreements", async (c) => {
  ...
  if (agreed_responsible_use !== true || agreed_visibility_acknowledgment !== true || agreed_community_guidelines !== true) {
  ...
  await c.env.DB.prepare(
    `UPDATE users SET
     ...
     WHERE email = ?`
  ).bind(notifications_enabled === true ? 1 : 0, user.email).run();
...
userRoutes.post("/me/profile", async (c) => {
  ...
  const dbUser = await c.env.DB.prepare("SELECT id FROM users WHERE email = ?").bind(user.email).first();
...
userRoutes.get("/me", async (c) => {
  ...
  if (!user) return c.json({ error: "UNAUTHORIZED" }, 401);
  const dbUser = await c.env.DB.prepare("SELECT * FROM users WHERE email = ?").bind(user.email).first();
  if (!dbUser) return c.json({ error: "USER_NOT_FOUND" }, 404);
  const userId = dbUser.id;
...
// .wrangler/tmp/bundle-uzOIZH/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-uzOIZH/middleware-loader.entry.ts
